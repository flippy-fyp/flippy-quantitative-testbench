...
def ms_to_tick_gap_and_note_off(
    notes: List[Tuple[float, int]], offset_ticks: int
) -> List[Tuple[int, int]]:
    ...
    off_notes_ticks = [
        (t + offset_ticks, -midi_note_num) for (t, midi_note_num) in notes_ticks
    ]
...
def process_notes(notes: List[Tuple[float, int]], offset_ticks: int) -> MidiFile:
    ...
    # first convert to absolute ticks
    notes_ticks = ms_to_tick_gap_and_note_off(notes, offset_ticks)
